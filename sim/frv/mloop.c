...
static INLINE void
frvbf_fill_argbuf (const SIM_CPU *cpu, ARGBUF *abuf, const IDESC *idesc,
...
static INLINE void
frvbf_fill_argbuf_tp (const SIM_CPU *cpu, ARGBUF *abuf,
...
frvbf_emit_before (SIM_CPU *current_cpu, SCACHE *sc, PCADDR pc, int first_p)
{
  ARGBUF *abuf = &sc[0].argbuf;
  const IDESC *id = & CPU_IDESC (current_cpu) [FRVBF_INSN_X_BEFORE];

  abuf->fields.before.first_p = first_p;
  frvbf_fill_argbuf (current_cpu, abuf, id, pc, 0);
...
ATTRIBUTE_UNUSED static INLINE void
frvbf_emit_after (SIM_CPU *current_cpu, SCACHE *sc, PCADDR pc)
{
  ARGBUF *abuf = &sc[0].argbuf;
  const IDESC *id = & CPU_IDESC (current_cpu) [FRVBF_INSN_X_AFTER];
...
#line 43 "frv/mloop.in"

static INLINE const IDESC *
extract (SIM_CPU *current_cpu, PCADDR pc, CGEN_INSN_INT insn, ARGBUF *abuf,
         int fast_p)
{
  const IDESC *id = frvbf_decode (current_cpu, pc, insn, insn, abuf);
  frvbf_fill_argbuf (current_cpu, abuf, id, pc, fast_p);
  if (! fast_p)
    {
      int trace_p = PC_IN_TRACE_RANGE_P (current_cpu, pc);
      int profile_p = PC_IN_PROFILE_RANGE_P (current_cpu, pc);
      frvbf_fill_argbuf_tp (current_cpu, abuf, trace_p, profile_p);
    }
  return id;
}
...
execute (SIM_CPU *current_cpu, SCACHE *sc, int fast_p)
{
  ...
  frvbf_h_gr_set (current_cpu, 0, 0);
  ...
  if (fast_p)
      ...
      if (! virtual_p)
	{
	  ...
	  if (FRV_COUNT_CYCLES (current_cpu, ARGBUF_PROFILE_P (abuf)))
	    {
	      frvbf_model_insn_before (current_cpu, sc->first_insn_p);
      ...
      if (! virtual_p)
	{
	  ...
	  if (FRV_COUNT_CYCLES (current_cpu, ARGBUF_PROFILE_P (abuf)))
		...
		cycles = 1;
	      frvbf_model_insn_after (current_cpu, sc->last_insn_p, cycles);
	    }
...
static void
frvbf_parallel_write_init (SIM_CPU *current_cpu)
{
...
static void
frvbf_parallel_write_queued (SIM_CPU *current_cpu)
{
  int i;
...
void
frvbf_perform_writeback (SIM_CPU *current_cpu)
{
  frvbf_parallel_write_queued (current_cpu);
}
...
static void
frvbf_simulate_insn_prefetch (SIM_CPU *current_cpu, IADDR vpc)
{
  SI hsr0;
  SIM_DESC sd;
...
#line 707 "./common/genmloop.sh"

static INLINE SCACHE *
frvbf_scache_lookup (SIM_CPU *current_cpu, PCADDR vpc, SCACHE *scache,
...
#line 780 "./common/genmloop.sh"

  if (! CPU_IDESC_SEM_INIT_P (current_cpu))
    {
#if ! WITH_SEM_SWITCH_FULL
      frvbf_sem_init_idesc_table (current_cpu);
#endif
...
#line 456 "frv/mloop.in"
{
  ...
  frvbf_parallel_write_init (current_cpu);
  ...
  for (ninsns = 0; ! last_insn_p && ninsns < FRV_VLIW_SIZE; ++ninsns)
    {
      ...
      frvbf_simulate_insn_prefetch (current_cpu, vpc);

      sc = frvbf_scache_lookup (current_cpu, vpc, scache, hash_mask, FAST_P);
