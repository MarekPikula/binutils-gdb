...
static INLINE void
m32r2f_fill_argbuf (const SIM_CPU *cpu, ARGBUF *abuf, const IDESC *idesc,
...
static INLINE void
m32r2f_fill_argbuf_tp (const SIM_CPU *cpu, ARGBUF *abuf,
...
m32r2f_emit_before (SIM_CPU *current_cpu, SCACHE *sc, PCADDR pc, int first_p)
{
  ARGBUF *abuf = &sc[0].argbuf;
  const IDESC *id = & CPU_IDESC (current_cpu) [M32R2F_INSN_X_BEFORE];

  abuf->fields.before.first_p = first_p;
  m32r2f_fill_argbuf (current_cpu, abuf, id, pc, 0);
...
ATTRIBUTE_UNUSED static INLINE void
m32r2f_emit_after (SIM_CPU *current_cpu, SCACHE *sc, PCADDR pc)
{
  ARGBUF *abuf = &sc[0].argbuf;
  const IDESC *id = & CPU_IDESC (current_cpu) [M32R2F_INSN_X_AFTER];
	 ...
	 SCACHE *sc, int fast_p, int parallel_p)
{
  ...
  const IDESC *id = m32r2f_decode (current_cpu, pc, insn, insn, abuf);

  if (parallel_p)
    id = id->par_idesc;
  m32r2f_fill_argbuf (current_cpu, abuf, id, pc, fast_p);
  return id;
}
	     ...
	     int trace_p, int profile_p)
{
  ...
  m32r2f_emit_before (current_cpu, sc, pc, 1);
  id = emit_16 (current_cpu, pc, insn, sc + 1, 0, 0);
  m32r2f_emit_after (current_cpu, sc + 2, pc);
  m32r2f_fill_argbuf_tp (current_cpu, &sc[1].argbuf, trace_p, profile_p);
  return id;
}
		    ...
		    SCACHE *sc, int trace_p, int profile_p)
{
  ...
  m32r2f_emit_before (current_cpu, sc + 2, pc, 0);

  /* Note that this can never be a cti.  No cti's go in the S pipeline.  */
  id2 = emit_16 (current_cpu, pc + 2, insn & 0x7fff, sc + 3, 0, 0);
  m32r2f_fill_argbuf_tp (current_cpu, &sc[3].argbuf, trace_p, profile_p);
  ...
  m32r2f_emit_after (current_cpu, sc + 5, pc);
...
{
  ARGBUF *abuf = &sc->argbuf;
  const IDESC *id = m32r2f_decode (current_cpu, pc,
...
INLINE SEM_PC
m32r2f_pbb_begin (SIM_CPU *current_cpu, int FAST_P)
{
  SEM_PC new_vpc;
  PCADDR pc;
  SCACHE *sc;
...
/* end extract-pbb */
	...
	if (_cti_sc)
	  {
	    id = & CPU_IDESC (current_cpu) [M32R2F_INSN_X_CTI_CHAIN];
	  }
	else
	  {
	    id = & CPU_IDESC (current_cpu) [M32R2F_INSN_X_CHAIN];
	  }
...
INLINE SEM_PC
m32r2f_pbb_chain (SIM_CPU *current_cpu, SEM_ARG sem_arg)
{
...
INLINE SEM_PC
m32r2f_pbb_cti_chain (SIM_CPU *current_cpu, SEM_ARG sem_arg,
...
void
m32r2f_pbb_before (SIM_CPU *current_cpu, SCACHE *sc)
{
  SEM_ARG sem_arg = sc;
	  ...
	  if (ARGBUF_PROFILE_P (prev_abuf))
	    {
	      ...
	      m32r2f_model_insn_after (current_cpu, 0 /*last_p*/, cycles);
	    }
	}
  ...
  if (PROFILE_MODEL_P (current_cpu)
    ...
    m32r2f_model_insn_before (current_cpu, first_p);
...
void
m32r2f_pbb_after (SIM_CPU *current_cpu, SCACHE *sc)
{
  const SEM_ARG prev_sem_arg = sc - 1;
  const ARGBUF *prev_abuf = SEM_ARGBUF (prev_sem_arg);
  ...
  if (PROFILE_MODEL_P (current_cpu)
      && ARGBUF_PROFILE_P (prev_abuf))
    {
      ...
      cycles = (*prev_idesc->timing->model_fn) (current_cpu, prev_sem_arg);
      m32r2f_model_insn_after (current_cpu, 1 /*last_p*/, cycles);
    }
...
void
m32r2f_engine_run_full (SIM_CPU *current_cpu)
{
...
#endif
#else
      m32r2f_sem_init_idesc_table (current_cpu);
#endif

      /* Initialize the "begin (compile) a pbb" virtual insn.  */
      vpc = CPU_SCACHE_PBB_BEGIN (current_cpu);
      SEM_SET_FULL_CODE (SEM_ARGBUF (vpc),
			 & CPU_IDESC (current_cpu) [M32R2F_INSN_X_BEGIN]);
      vpc->argbuf.idesc = & CPU_IDESC (current_cpu) [M32R2F_INSN_X_BEGIN];
