...
static INLINE void
or1k32bf_fill_argbuf (const SIM_CPU *cpu, ARGBUF *abuf, const IDESC *idesc,
...
static INLINE void
or1k32bf_fill_argbuf_tp (const SIM_CPU *cpu, ARGBUF *abuf,
...
or1k32bf_emit_before (SIM_CPU *current_cpu, SCACHE *sc, PCADDR pc, int first_p)
{
  ARGBUF *abuf = &sc[0].argbuf;
  const IDESC *id = & CPU_IDESC (current_cpu) [OR1K32BF_INSN_X_BEFORE];

  abuf->fields.before.first_p = first_p;
  or1k32bf_fill_argbuf (current_cpu, abuf, id, pc, 0);
...
ATTRIBUTE_UNUSED static INLINE void
or1k32bf_emit_after (SIM_CPU *current_cpu, SCACHE *sc, PCADDR pc)
{
  ARGBUF *abuf = &sc[0].argbuf;
  const IDESC *id = & CPU_IDESC (current_cpu) [OR1K32BF_INSN_X_AFTER];
...
{
  const IDESC *id = or1k32bf_decode (current_cpu, pc, insn, insn, abuf);
  or1k32bf_fill_argbuf (current_cpu, abuf, id, pc, fast_p);
  if (!fast_p)
    {
      int trace_p = PC_IN_TRACE_RANGE_P (current_cpu, pc);
      int profile_p = PC_IN_PROFILE_RANGE_P (current_cpu, pc);
      or1k32bf_fill_argbuf_tp (current_cpu, abuf, trace_p, profile_p);
    }
  return id;
}
...
execute (SIM_CPU *current_cpu, SCACHE *sc, int fast_p)
{
  ...
  or1k32bf_insn_before (current_cpu, vpc, sc->argbuf.idesc);
      ...
      if (! virtual_p)
        {
          ...
          if (PROFILE_MODEL_P (current_cpu)
            ...
            or1k32bf_model_insn_before (current_cpu, 1 /*first_p*/);
      ...
      if (! virtual_p)
        {
          ...
          if (PROFILE_MODEL_P (current_cpu)
              && ARGBUF_PROFILE_P (abuf))
            {
              ...
              or1k32bf_model_insn_after (current_cpu, 1 /*last_p*/, cycles);
            }
  ...
  or1k32bf_insn_after (current_cpu, vpc, sc->argbuf.idesc);
...
INLINE SEM_PC
or1k32bf_pbb_begin (SIM_CPU *current_cpu, int FAST_P)
{
  SEM_PC new_vpc;
  PCADDR pc;
  SCACHE *sc;
...
/* end extract-pbb */
	...
	if (_cti_sc)
	  {
	    id = & CPU_IDESC (current_cpu) [OR1K32BF_INSN_X_CTI_CHAIN];
	  }
	else
	  {
	    id = & CPU_IDESC (current_cpu) [OR1K32BF_INSN_X_CHAIN];
	  }
...
INLINE SEM_PC
or1k32bf_pbb_chain (SIM_CPU *current_cpu, SEM_ARG sem_arg)
{
...
INLINE SEM_PC
or1k32bf_pbb_cti_chain (SIM_CPU *current_cpu, SEM_ARG sem_arg,
...
void
or1k32bf_pbb_before (SIM_CPU *current_cpu, SCACHE *sc)
{
  SEM_ARG sem_arg = sc;
	  ...
	  if (ARGBUF_PROFILE_P (prev_abuf))
	    {
	      ...
	      or1k32bf_model_insn_after (current_cpu, 0 /*last_p*/, cycles);
	    }
	}
  ...
  if (PROFILE_MODEL_P (current_cpu)
    ...
    or1k32bf_model_insn_before (current_cpu, first_p);
...
void
or1k32bf_pbb_after (SIM_CPU *current_cpu, SCACHE *sc)
{
  const SEM_ARG prev_sem_arg = sc - 1;
  const ARGBUF *prev_abuf = SEM_ARGBUF (prev_sem_arg);
...
#endif
#else
      or1k32bf_sem_init_idesc_table (current_cpu);
#endif

      /* Initialize the "begin (compile) a pbb" virtual insn.  */
      vpc = CPU_SCACHE_PBB_BEGIN (current_cpu);
      SEM_SET_FULL_CODE (SEM_ARGBUF (vpc),
			 & CPU_IDESC (current_cpu) [OR1K32BF_INSN_X_BEGIN]);
      vpc->argbuf.idesc = & CPU_IDESC (current_cpu) [OR1K32BF_INSN_X_BEGIN];
...
void
or1k32bf_engine_run_fast (SIM_CPU *current_cpu)
{
  /* virtual program counter */
  SEM_PC vpc;
#if WITH_SEM_SWITCH_FAST
...
#endif
#else
      or1k32bf_semf_init_idesc_table (current_cpu);
#endif
